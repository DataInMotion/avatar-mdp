...
public class GDPRModelEvaluator implements ModelEvaluator {
	...
	public EvaluationSummary evaluateModel(EPackage ePackage) {
        ...
        try {
			...
			LOGGER.log(Level.SEVERE, String.format("Exception while making GDPR predicitons for model %s", ePackage.getName()), e);
        }
		return null;
	}
		...
		modelPredictions.forEach((k,v) -> {
			...
			predictions.forEach((doc, pred) -> {
				...
				pred.forEach((category, level) -> {
					if(NEGATION_DETECTED_CAT.equals(category)) {
						evaluation.setNegationDetected(Boolean.valueOf(level));
					} else {
